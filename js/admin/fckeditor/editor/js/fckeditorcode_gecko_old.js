...
var FCK = {
    ...
    InsertElement: function (A) {
        ...
        if (FCKListsLib.BlockElements[B] != null) {
            ...
            else C.MoveToPosition(A, 4); if (FCKBrowserInfo.IsGeckoLike) {
...
var FCKDomTools = {
    ...
    ListToArray: function (A, B, C, D, E) {
        ...
        for (var i = 0; i < A.childNodes.length; i++) {
            ...
            } else G.grandparent = E; if (B) this.SetElementMarker(B, F, '_FCK_ListArray_Index', C.length);
            C.push(G);
...
FCKXHtml.GetXHTML = function (A, B, C) {
    ...
    else E = E.replace(FCKRegexLib.SpaceNoClose, ' />'); if (FCKConfig.ForceSimpleAmpersand) E = E.replace(FCKRegexLib.ForceSimpleAmpersand, '&');
...
FCKXHtml._AppendNode = function (A, B) {
    ...
    case 1:
        ...
        if (F) E = F(E, B, A);
        else E = this._AppendChildNodes(E, B, Boolean(FCKListsLib.NonEmptyBlockElements[D])); if (!E) return false;
        A.appendChild(E);
        break;
    case 3:
    ...
    OutdentListItem: function (A) {
        ...
        if (B.tagName.toUpperCase().Equals('UL', 'OL')) {
            ...
            if (L || K.Equals('UL', 'OL')) {
                ...
                else D.InsertAfterNode(B); if (L) FCKDomTools.InsertAfterNode(B.parentNode, B.removeChild(A));
...
FCKDomRange.prototype = {
    ...
    CheckEndOfBlock: function (A) {
        ...
        } else F = D.childNodes[E]; if (!F) F = FCKDomTools.GetNextSourceNode(D, true, null, C);
        var H = false;
        while (F) {
            switch (F.nodeType) {
            case 1:
        ...
        while (H) {
            ...
            if (!K) {
                ...
                if (F[M] && (!FCKBrowserInfo.IsIE || H.scopeName == 'HTML')) {
                    ...
                    if (B) {
                        B.SetEnd(H, 3, true);
                        if (M != 'br') this._NextNode = FCKDomTools.GetNextSourceNode(H, true, null, I);
                    };
                    J = true;
                } else {
                    if (H.firstChild) {
...
FCKEnterKey.prototype._ExecuteEnterBlock = function (A, B) {
    ...
    if (D) {
        ...
        }; if (FCKBrowserInfo.IsGeckoLike) FCKDomTools.ScrollIntoView(F || I, false);
        C.Select();
    };
    C.Release();
    return true;
...
FCKEnterKey.prototype._ExecuteEnterBr = function (A) {
    ...
    if (B.StartBlockLimit == B.EndBlockLimit) {
        ...
        if (!F && D && (/^H[1-6]$/).test(E)) {
            ...
            else B.SetStart(H.nextSibling, 1); if (!FCKBrowserInfo.IsIE) {
                var I = null;
...
FCKTableHandler.InsertColumn = function (A) {
    ...
    for (var i = 0; i < D.rows.length; i++) {
        ...
        var G = F.cells[E];
        if (A) F.insertBefore(B, G);
        else if (G.nextSibling) F.insertBefore(B, G.nextSibling);
...
FCKTableHandler._UnmarkCells = function (A, B) {
    for (var i = 0; i < A.length; i++) {
        if (FCKBrowserInfo.IsIE) A[i].removeAttribute(B);
        else delete A[i][B];
    }
};
...
FCKTableHandler.VerticalSplitCell = function () {
    ...
    if (E > 1) {
        ...
        if (L.rows.length > G) L.insertBefore(K, L.rows[G]);
...
FCKTableHandler._InstallTableMap = function (A, B) {
    ...
    for (var i = 0; i < A.length; i++) {
        for (var j = 0; j < A[i].length; j++) {
            ...
            if (!E) continue;
            if (j > F) F = j;
            if (E._colScanned === true) continue;
            if (A[i][j - 1] == E) E.colSpan++;
            if (A[i][j + 1] != E) E._colScanned = true;
        }
    };
    for (var i = 0; i <= F; i++) {
        for (var j = 0; j < A.length; j++) {
            if (!A[j]) continue;
            var E = A[j][i];
            if (!E || E._rowScanned === true) continue;
            if (A[j - 1] && A[j - 1][i] == E) E[C]++;
            if (!A[j + 1] || A[j + 1][i] != E) E._rowScanned = true;
        }
    };
    for (var i = 0; i < A.length; i++) {
        for (var j = 0; j < A[i].length; j++) {
    ...
    for (var i = 0; i < A.length; i++) {
        ...
        for (var j = 0; j < A[i].length;) {
            ...
            j += E.colSpan;
            if (E.colSpan == 1) E.removeAttribute('colspan');
            if (E.rowSpan == 1) E.removeAttribute('rowspan');
        };
        B.appendChild(J);
    }
};
...
FCKTableHandler.GetSelectedCells = function () {
    ...
    for (var i = 0; i < B.rangeCount; i++) {
        ...
        else E = D.startContainer.childNodes[D.startOffset]; if (E.tagName.Equals('TD', 'TH')) A[A.length] = E;
    };
    return A;
};
var FCKXml = function () {
...
FCKShowBlockCommand.prototype.Execute = function () {
    ...
    else B.className += ' FCK__ShowBlocks'; if (FCKBrowserInfo.IsIE) {
        try {
...
FCKCommands.GetCommand = function (A) {
    ...
    case 'Flash':
        ...
        break;
    case 'UpFileBtn':
        B = new FCKDialogCommand('UpFileBtn', FCKLang.UpFileBtn, 'dialog/fck_UpFileBtn.html', 400, 300);
        break;
    case 'SpecialChar':
...
FCKToolbarButtonUI.prototype.Create = function (A) {
    ...
    if (this.Style == 0 && !this.ShowArrow) {
        ...
        else F.appendChild(this._CreatePaddingElement(B)); if (this.Style == 1 || this.Style == 2) {
            F = E.insertCell(-1);
...
FCKToolbarSpecialCombo.prototype.RefreshState = function () {
    ...
    } else A = -1; if (A == this.State) return;
    if (A == -1) {
        this._Combo.DeselectAll();
...
FCKToolbarItems.GetItem = function (A) {
    ...
    case 'Flash':
        ...
        break;
    case 'UpFileBtn':
        B = new FCKToolbarButton('UpFileBtn', FCKLang.UpFileBtn, FCKLang.UpFileBtn, null, false, true, 4);
        break;
    case 'Table':
